...
|---|---|---|
| **0 — Setup** | Install .NET 10 SDK, scaffold solution/projects above, git init, NuGet (CredentialManagement, xUnit, Moq, Azure.Storage.Blobs). Azure Storage account (`mrphilbookworm`) already created — confirm the `memory` container exists, then load the saved connection string into `ICredentialStore`. | No |
