...
**Reading history, preferred authors, preferred genres** (added after Phase 5): three more lists on `BookwormMemory`, all explicitly requested rather than inferred —

- `ReadingHistory` (`List<ReadingHistoryEntry>`): every title ever added to the bookshelf via `add_to_bookshelf`, independent of VA's own bookshelf/loan history (so it survives a title being removed, and covers titles no longer on loan). Each entry has title, author, bookshareId, `DateAdded`, an optional `DateRemoved` (set when `remove_from_bookshelf` is called with a matching `bookshareId`), and an optional 1-5 `Rating`. Exposed to the Brain via `search_reading_history` (substring match on title/author) and `rate_book` (matches by title, rejects ambiguous multi-title matches). Deliberately left out of the system-prompt memory snapshot (only its count is included) since it can grow unbounded — it's a search tool, not baseline context.
- `PreferredAuthors` (`List<AuthorPreference>`): authors the user has confirmed, each with an `IsFavorite` flag. Never auto-populated — `add_to_bookshelf`'s result includes `authorAlreadyInPreferredAuthors`, and persona rule 10a has the Brain ask once per new author (favorite or not) before calling `add_preferred_author`.
- `PreferredGenres` (`List<string>`): same pattern — `add_to_bookshelf`'s result includes `currentPreferredGenres`, and persona rule 10b has the Brain propose a genre (from its own knowledge of the title/author) and only call `add_preferred_genre` if the user agrees. Never inferred from bookshelf contents alone.

Both the new author-check and genre-check signals are computed fresh on every `add_to_bookshelf` call (not read from the possibly-stale system-prompt snapshot), including in `--dry-run` mode, so the ask-once-per-author / propose-genre conversational flow can be exercised without touching the real VA account.
