...
- `POST /library/quick-search`, **form-urlencoded** (not JSON) body: `keyword`, `type` (one of `Book`, `Picture Book`, `Magazine`, `Newspaper`, `Podcast`, `Music` — exact dropdown values), `limit`, `format` (a formatId like `DAISY_Audio_Human`, or empty for "All formats"). Returns JSON: `bookTab`/`periodicalTab`/`musicTab`, each with `total` and `listArticle[]` (title, authors[], formats[], size — string for books, **numeric 0 for periodicals**, bookshareId, activeTitleId, status.key e.g. `READY_FOR_DOWNLOAD`, description). The portal's own UI only ever sends `limit=10` (its search box has no page control), but the endpoint honors much higher values fine — confirmed live up to 100 with no server-side cap observed, returning up to `total` either way. `VaLibraryClient` requests 50 as a deliberate middle ground (covers nearly all real searches completely without an uncapped/unbounded payload), not the server's ceiling. No page/offset parameter was found for this endpoint, so true pagination beyond one call isn't implemented — `ResultSummarizer` surfaces `shown` vs `total` explicitly so the Brain knows when to narrow the conversation instead of pretending it has everything.
